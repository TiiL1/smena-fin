...
Новое в этой версии:
- **Доп. доход.** Подработка, курьерка, продажи — отдельной сущностью
  «Доход»: кнопки «Доход»/«Трата» на вкладке «Деньги», источники,
  дата задним числом, общая лента движения денег.
- **Доход прямо боту.** Сообщение вида `+15000 курьерка` сразу падает
  в свободный баланс, симметрично тратам `-3500 еда`.
- **Подработка в разборе недели.** Сколько заработано за 7 дней,
  сколько записей, топ-источник.
- **Возврат из цели в баланс.** На карточке цели кнопка «Вернуть
  в баланс» с вводом суммы и кнопкой «Всё» — обратный путь после сплита.
- **Подтверждение сплита.** Кнопка «Сплит по целям» сначала показывает
  разбивку по каждой цели в рублях, распределяет только по второму тапу.
- Новые таблицы в базе (`incomes`, плюс ранее `expenses`, `fixed_costs`)
  создадутся сами при следующем деплое — руками в Neon ничего делать
  не нужно.
...
```bash
git add -A
git commit -m "Доп. доход, возврат из цели, подтверждение сплита"
git push
```
