...
Новое в этой версии:
- **Метки и автокатегоризация.** Бот понимает текст траты: `-5155 ресторан`
  → категория «Еда», метка «Ресторан»; `-110 автобус` → «Транспорт»,
  метка «Автобус». API добавлена колонка `tag` в `expenses`/`incomes`.
- **Тихий перерасход** на главной карточке: сколько денег «залочено»
  под цели и обязательные расходы — остаток должен хватить до зарплаты.
- **Сценарии цели** (вариант C): чипы «+2 смены», «−20k расходов»,
  «сначала закрыть долг» показывают новую дату достижения без правок цели.
- **Недельная аналитика трат** (вариант A): карточка «Последняя неделя»
  со средним дневным расходом, самым дорогим днём, топ-категорией и
  движением по дням недели.
- **Аудит зарплаты** (вариант B): на странице «Статистика» блок показывает
  начисления (смены × ставки) vs реальные выплаты за 3 месяца, подсвечивает
  расхождения.
- **Анализ по дням недели и меткам** на StatsPage: отдельные блоки
  «Траты по дням недели» и «Детали по меткам» (раскрытие тегов внутри
  категории).
- Новая таблица не создаётся — только `tag TEXT` добавится в существующие
  `expenses`/`incomes` через миграцию; Alembic не нужен (ручная `ALTER TABLE`
  перед деплоем, если base уже есть).
...
```bash
git add -A
git commit -m "Метки, автокатегоризация, аналитика, аудит зарплаты"
git push
```
